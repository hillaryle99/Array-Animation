...
function loopThrough(arr) {
  ...
  nextButton.onclick = () => {
    ...
    const cnt = Number(arr.innerHTML) + arr[i]
    arr.innerHTML = cnt
    const pre = cnt - arr[i]
    document.getElementById('icon-button').style.display = 'block';
    indexInfo.innerHTML = `sum = 0 <br><br> for (int index = 0; index < array.length; index++) <br><br>       sum += array[index] <br><br><br> Previous sum of the array is <span style='background-color:yellow'><b> ${pre}</b></span> <br><br><br> Current index is <span style='background-color:yellow'><b> ${i}</b></span> <br><br><br> Value of the current element is <span style='background-color:yellow'><b> ${arr[i]}</b></span> <br><br><br> Sum of the array is: <span style='background-color:yellow'><b> ${cnt} </b></span>`;
